...
class CameraProcessor:
    ...
    def load_image(self, image_path: str, color_mode: str = 'BGR') -> np.ndarray:
        ...
        try:
            if color_mode.upper() == 'GRAY':
                image = cv2.imread(image_path, xxxxxxxx) # TODO: cv2.IMREAD_GRAYSCALE 사용
            else:
    ...
    def edge_detection_sobel(self, image: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        # 절댓값으로 변환
        sobel_x = xxxxxx # TODO: 절댓값 함수 적용
        sobel_y = xxxxxx # TODO: 절댓값 함수 적용
    ...
    def lane_detection_pipeline(self, image: np.ndarray) -> Tuple[np.ndarray, np.ndarray, dict]:
        ...
        # 1단계: 그레이스케일 변환
        if len(image.shape) == 3:
            gray = xxxxxx # TODO: 컬러 이미지를 그레이스케일로 변환
        else:
            gray = image
        results['gray'] = gray
        ...
        # 3단계: Canny 엣지 검출
        edges = xxxxxx # TODO: Canny 함수 사용 - cv2.Canny
        results['edges'] = edges
    ...
    def detect_lanes_bev(self, bev_image: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        # 결과 이미지 생성
        result = xxxxxx # TODO: 그레이스케일 이미지를 3채널로 변환
